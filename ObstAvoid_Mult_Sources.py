...
#             f[j] = -2 * targPos[i]
    ...
    if len(transform) != 0:
        ...
        #print(dir0)
        firstHalf = dir0[:d]
        if np.linalg.norm(firstHalf) != 0:
            firstHalf /= np.linalg.norm(firstHalf)
        secHalf = dir0[d:]
        if np.linalg.norm(secHalf) != 0:
            secHalf /= np.linalg.norm(secHalf)
        dir0 = np.hstack((firstHalf, secHalf))
        pointSum = 0
        for i in range(n):
            ADelta[i, (lenX-2)*n + i] = 1
            for j in range(2*d):
                ADelta[i, j*n + i] = -transform[j] - transform[j + 2*d] * dir0[j]
                pointSum += transform[j] * qo[j] + transform[j + 2*d] * dir0[j] * qo[j]
